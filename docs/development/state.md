...
- **Cyrius**: `6.0.14` (pinned in `cyrius.cyml [package].cyrius`)
...
| Module count          | 12        |                                    |
| Test files            | 14        | tests/tcyr/                        |
| Test assertions       | 851       | 0 failed; +66 byte-exact OTLP coverage in v1.2.0; unchanged at v1.2.1/v1.2.2/v1.2.3 (toolchain refreshes) |
| Benchmarks            | 25        | tests/bcyr/                        |
| Test binary (DCE)     | ~306 KB   | `build/agnostik` after `CYRIUS_DCE=1 cyrius build` (261→273 KB at 1.0.2; 274 KB at 1.0.3+; ~311 KB at 1.2.0 from chrono+proto surface; ~304 KB at 1.2.1 from 5.10.34 codegen shrink; ~305 KB at 1.2.2 +1 KB drift; ~306 KB / 313,344 B at 1.2.3 across the 6.0.x boundary) |
| Build warnings        | 0         |                                    |
...
See [`CHANGELOG.md`](../../CHANGELOG.md). Most recent stable: `1.2.3` (major-toolchain refresh — Cyrius `5.10.44` → `6.0.14`, first 6.x pin; stdlib workflow moved `cyrius deps` → `cyrius lib sync`; v1.2.1/1.2.2 hot-path wins held, no agnostik-side source changes).
