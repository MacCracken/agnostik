...
- **Cyrius**: `5.10.44` (pinned in `cyrius.cyml [package].cyrius`)
...
| Test assertions       | 851       | 0 failed; +66 byte-exact OTLP coverage in v1.2.0; unchanged at v1.2.1/v1.2.2 (toolchain refreshes) |
| Benchmarks            | 25        | tests/bcyr/                        |
| Test binary (DCE)     | ~305 KB   | `build/agnostik` after `CYRIUS_DCE=1 cyrius build` (261→273 KB at 1.0.2; 274 KB at 1.0.3+; ~311 KB at 1.2.0 from chrono+proto surface; ~304 KB at 1.2.1 from 5.10.34 codegen shrink; ~305 KB at 1.2.2 +1 KB drift) |
| Build warnings        | 0         |                                    |
...
See [`CHANGELOG.md`](../../CHANGELOG.md). Most recent stable: `1.2.2` (toolchain refresh — Cyrius `5.10.34` → `5.10.44`; v1.2.1 hot-path wins held, `accelerator_device_full` −12% additional, no agnostik-side source changes).
