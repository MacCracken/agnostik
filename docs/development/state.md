...
- **Cyrius**: `5.7.12` (pinned in `cyrius.cyml [package].cyrius`)
...
- **`cyrius audit`** is broken on a fresh 5.7.12 install (missing
  `check.sh` — filed in
...
| Test binary (DCE)     | 214 KB    | `build/agnostik` after `CYRIUS_DCE=1 cyrius build` |
| Build warnings        | 0         |                                    |
