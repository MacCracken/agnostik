...
- **Cyrius**: `5.10.34` (pinned in `cyrius.cyml [package].cyrius`)
...
| Test assertions       | 851       | 0 failed; +66 byte-exact OTLP coverage in v1.2.0; unchanged at v1.2.1 (toolchain refresh) |
| Benchmarks            | 25        | tests/bcyr/                        |
| Test binary (DCE)     | ~304 KB   | `build/agnostik` after `CYRIUS_DCE=1 cyrius build` (261→273 KB at 1.0.2; 274 KB at 1.0.3+; ~311 KB at 1.2.0 from chrono+proto surface; ~304 KB at 1.2.1 from 5.10.34 codegen shrink) |
| Build warnings        | 0         |                                    |
...
See [`CHANGELOG.md`](../../CHANGELOG.md). Most recent stable: `1.2.1` (toolchain refresh — Cyrius `5.10.20` → `5.10.34`; codegen-driven hot-path wins, no agnostik-side source changes).
