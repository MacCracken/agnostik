...
| `SECURITY.md` | 2026-09-23 | 🟡 Stale — Supported Versions | Audits table now lists all five reports (it had stopped at 2026-05-10), and the identifier-generation scope item notes F-022/F-023. **Still stale:** the Supported Versions table calls 1.2.x current. Which lines get security backports is the maintainer's policy call, so it was not edited. Earlier: Supported lines (1.0/1.1/1.2), scope by attack surface, audits table. |
...
| `2026-09-23-audit.md` | 2026-09-23 | ✅ Fresh | v1.6.2 **cross-target syscall review** (cyrius `6.6.2 → 6.6.6`), the first pass to judge `syscall(` sites against every target the toolchain builds. **F-022 (MEDIUM)**: raw `getrandom` 318 was `-ENOSYS` on aarch64 and not a syscall on agnos; fixed via `sys_getrandom` and verified under `qemu-aarch64 -strace`. **F-023 (LOW, open)**: the fallback's raw numbers are wrong on agnos; two fix options recorded. Also records why CI never saw it (the `cc5_aarch64` probe). |

Next audit slot: the next minor cut, where F-023's fix is the natural candidate, or sooner if a CVE pattern surfaces in agnostik's input-handling paths or the cyrius toolchain's parser/serde dependencies.
