...
| `CHANGELOG.md` | 2026-08-24 | ✅ Fresh | Source of truth for shipped work. Through **1.3.6** (cyrius `6.5.27 → 6.5.35`; Toolchain + Fixed + Performance sections; 25 checked / 0 regressions, 23 of 25 faster; `agnostik.bcyr` proto-include fix; fmt debt cleared; binary 413,512 → 629,032 B). `[Unreleased]` empty. Note **1.3.5**'s entry is a single line with a `354,112 -> 417,608 bytes` figure that does not reproduce — superseded by the 1.3.6 measurements, left as shipped. |
