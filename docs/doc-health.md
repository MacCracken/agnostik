...
> **Last refresh**: 2026-05-09 (initial audit, paired with v1.2.0 doc sweep) | **Refresh cadence**: when docs are touched, update the affected row.
...
|---|---|---|---|
| `README.md` | 2026-05-09 | ✅ Fresh | Substantial rewrite in v1.2.0 audit. Status block + module table + OTLP example + gate-script list + Decisions section. |
| `CHANGELOG.md` | 2026-05-09 | ✅ Fresh | Source of truth for shipped work. Updated through 1.2.0. |
...
| `VERSION` | 2026-05-09 | ✅ Fresh | `1.2.0` — source of truth, matches `cyrius.cyml`. |
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `state.md` | 2026-05-09 | ✅ Fresh | Live volatile state (version, sizes, test count, consumers, verification hosts). Refreshed every release. |
| `roadmap.md` | 2026-05-09 | ✅ Fresh | Trimmed in v1.2.0 audit — only forward pins remain (v1.2.1, v1.2.2, v2.0.0). CHANGELOG owns shipped history. |
...
| `cyrius-audit-missing-check-script-2026-04-26.md` | 2026-05-03 | 🟠 Read-through | Filed against 5.7.x; toolchain is now 5.10.20. **Verify** whether `~/.cyrius/bin/check.sh` ships in the current install — if so, archive. |
...
- `cyrius-audit-missing-check-script-2026-04-26.md` — needs upstream verification (5.10.20 vs 5.7.x). Owner: agnostik audit pass; trigger: next time `cyrius audit` is run on a fresh install.
...
*Last refresh: 2026-05-09 (initial audit, paired with v1.2.0 doc sweep). Refresh in place when docs are touched.*
